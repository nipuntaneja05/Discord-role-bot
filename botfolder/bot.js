  ...
  try {
    ...
    for (const [, channel] of channels) {
      if (channel.isTextBased()) {
        ...
        const userMessages = messages.filter(msg => msg.author.id === userId).size;

        // ✅ Fix: Ensure messages are not double counted
        totalMessages = Math.max(totalMessages, userMessages);
      }
    }
...
async function assignRoles(roleData) {
  try {
    ...
    const condition = roleData.condition;
    const requiredMessages = parseInt(condition.match(/\d+/)[0]); // Extract number from condition
    const operator = condition.match(/(>=|<=|>|<|==|!=)/)[0]; // Extract the operator

    console.log(`🔹 Condition: messages ${operator} ${requiredMessages}`);
