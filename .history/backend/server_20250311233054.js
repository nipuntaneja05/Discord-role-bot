...
app.post("/process-command", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // ✅ Send JSON to bot for role assignment
      console.log("🔹 Forwarding role data to bot:", parsedResponse);
      await assignRoles(parsedResponse);

      return res.json({ success: "Role assignment in progress", roleData: parsedResponse });
    } catch (error) {
